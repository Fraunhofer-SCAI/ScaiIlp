...
namespace ilp_solver
{
    ...
    void ILPSolverGurobi::set_start_solution(ValueArray p_solution)
    {
        ...
        call_gurobi(d_model, GRBsetdblattrarray, d_model, GRB_DBL_ATTR_LB, 0, d_num_vars, p_solution.data());
        call_gurobi(d_model, GRBsetdblattrarray, d_model, GRB_DBL_ATTR_UB, 0, d_num_vars, p_solution.data());
        solve_impl();
        ...
        call_gurobi(d_model, GRBsetdblattrarray, d_model, GRB_DBL_ATTR_START, 0, d_num_vars, p_solution.data());
                                            ...
                                            OptionalValueArray p_row_values, OptionalIndexArray p_row_indices)
    {
        ...
        call_gurobi(d_model, GRBaddvar, d_model, num, indices, values, p_objective,
                    p_lower_bound, p_upper_bound, type, p_name.c_str());
        ++d_num_vars;
    }
                                              ...
                                              const std::string& p_name, OptionalIndexArray p_col_indices)
    {
        ...
        {
            call_gurobi(d_model, GRBaddconstr, d_model, num, indices, values, GRB_GREATER_EQUAL, p_lower_bound,
                        p_name.c_str());
            ++d_num_cons;
        }
        else if (p_lower_bound  < c_neg_inf_bound && p_upper_bound <= c_pos_inf_bound)
